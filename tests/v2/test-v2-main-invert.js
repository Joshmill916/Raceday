// Thorough test: main-event invert (feature + B-main starting order) — v2 port of
// tests/test-main-invert.js. Engine logic (invertTop/featureData/featureGridOrder/
// setClassMainInvert/clsFeatInvert) copied verbatim from v1 — only the DOM container
// changed: #gridWrap, not #gridContent.
const { chromium } = require('/opt/node22/lib/node_modules/playwright');
const http = require('http'); const fs = require('fs'); const path = require('path');
const ROOT = path.resolve(__dirname, '..', '..'); const PORT = 8819;
let pass = 0, fail = 0;
...
(async () => {
  const server = http.createServer((req, res) => {
    ...
    fs.readFile(path.join(ROOT, rel), (err, data) => {
      ...
      res.writeHead(200, { 'Content-Type': TYPES[path.extname(rel)] || 'text/plain' }); res.end(data);
    });
  }).listen(PORT);
  ...
  await page.evaluate(() => {
    ...
    for (let i = 1; i <= 12; i++) res['h1_' + (8000 + i)] = i;
    save();
    setDeviceRole('admin');
  ...
  // v2's default score-entry UI is tap mode (.tapcard .nm, plain text, no bold/.hr
  // wrapper) — a different DOM shape from v1's dropdown rows, same underlying order.
  let ftNames = await page.evaluate(() => {
    ...
    return [...fb.querySelectorAll('.tapcard .nm')].map(b => b.childNodes[0].textContent.trim());
  });
  check('P1 = D01 (no invert yet — placeholders open)', ftNames[0] === 'D01', ftNames.join(','));
  // Match on the invert phrase itself, not "B-main lineup" (v2's header is just "B-main")
  // — matching only "B-main" would also match the Standings block's "→ B-main" tags.
  const bmHeader = rows.find(t => t.includes('top 3 start inverted')) || '';
  ...
  let bmNames = await page.evaluate(() => {
    const blocks = [...document.querySelectorAll('#resBody .hb')];
    const bb = blocks.find(b => b.textContent.includes('top 3 start inverted'));
    return [...bb.querySelectorAll('.tapcard .nm')].map(b => b.childNodes[0].textContent.trim());
  });
  ...
  // The transfer-origin tag (transferOrigin(), computed identically to v1) only renders
  // in v2's dropdown score-entry mode (UI.scoreMode==='select') — tap mode (the default)
  // omits it from the tapcard to keep that view uncluttered. Switch modes to check it.
  const p7tag = await page.evaluate(() => {
    setScoreMode('select');
    const blocks = [...document.querySelectorAll('#resBody .hb')];
    const fb = blocks.find(b => b.textContent.includes('🏁 Feature'));
    const tag = fb.querySelectorAll('.lrow')[6].textContent;
    setScoreMode('tap');
    return tag;
  });
  ...
  await page.evaluate(() => { nav('grid'); setViewClass(null); });
  await page.waitForTimeout(300);
  const gridTxt = await page.textContent('#gridWrap');
  ...
  const gridFeatFirst = await page.evaluate(() => {
    const lbls = [...document.querySelectorAll('#gridWrap .slbl')];
  ...
  // v2's fan view has no separate .vw-body — renderFanView() shares #gridWrap and
  // gridMarkup() with the admin grid (that's the same "grid feature pole = D04" markup
  // just checked above, so the same selector applies).
  await page.evaluate(() => setDeviceRole('viewer'));
  await page.reload(); await page.waitForTimeout(300);
  await page.evaluate((cid) => { UI.viewClsId = cid; UI.viewStage = 'feat'; renderGrid(); }, clsId);
  await page.waitForTimeout(200);
  const vFirst = await page.evaluate(() => document.querySelector('#gridWrap .who b, #gridWrap .nm b').textContent);
  check('viewer feature pole = D04', vFirst === 'D04', vFirst);
  // v2's fan view deliberately drops the invert explanation text the admin grid shows
  // (confirmed at source: admin's "Feature lineup" label appends ftInvNote, the fan
  // view's doesn't) — spectators don't manage invert settings, so there's nothing to
  // explain. The order itself being correctly inverted (checked above) is what matters.
  const vLabel = await page.evaluate(() => document.querySelector('#gridWrap .slbl').textContent);
  check('viewer feature label is the plain "Feature lineup" (no invert note, by design)',
    vLabel.includes('Feature lineup') && !vLabel.includes('start inverted'), vLabel);
  await page.screenshot({ path: '/tmp/test-v2-invert-viewer.png' });
  ...
  console.log('\n— Admin UI + setter —');
  const admHtml = await page.evaluate(() => { nav('admin'); admOpen('classes'); renderClsList(); return document.getElementById('clsList').innerHTML; });
