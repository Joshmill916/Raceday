...
| `test-roles-security.js` | **Role-boundary + boot-sequence invariants** — what each role may see/do, the setup-wizard gating, `?role=` URL promotion, stuck-device recovery, forgotten-PIN recovery, sync write-blocks, the join-clobber warning, (§14/§15) that **viewer and tv devices can never trigger an admin-gated action even with no PIN configured** — `checkDayBanner()`'s day-banner visibility, `adminOk()` refusing viewer/tv outright before any PIN check, and the tv-role qualifying-times UI leak — and (§8b/§8c) that a **spectator joining a different track via QR never sees stale data from the old one**: no clobber confirm for viewers, `S.track`/`classes`/`roster`/`raceDay` cleared before the new room ever loads, and a clear "couldn't connect" message on an empty/misconfigured room instead of a silent stale grid — each verified independently via red/green mutation testing |
