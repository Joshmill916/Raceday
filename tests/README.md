...
| `test-standings-tiebreak.js` | `seriesStandings()`'s tie-break countback — tied points fall to wins, then best-ever feature finish, then alphabetical (not straight to alphabetical); a "Fix season points" repair night (no finish data at all) never unfairly outranks a driver with a real, if modest, recorded finish; two drivers with no finish data anywhere fall back to alphabetical cleanly |
